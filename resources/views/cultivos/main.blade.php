    ...
    @else
        @empty($datos)
            ...
            <div class="flex items-center justify-end mr-4">
                <a href="{{ route('cultivo.crear') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">@lang('app.nueCult')</a>
            </div>
        @else
            <script>document.title = "{{ config('app.name', 'Laravel') }} - Cultivos" </script>
            <h6 class="my-5 mx-4 text-2xl">@lang('app.lstCult')</h6>
            <div class="flex items-center justify-end mr-4">
                <a href="{{ route('cultivo.crear') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">@lang('app.nueCult')</a>
            </div>
                ...
                <table class="text-center text-2xl m-auto mt-5 bg-red-200 rounded-lg">
                    <thead>
                        <tr>
                            ...
                            <th class="px-6 py-4">@lang('app.des')</th>
                            <th class="px-6 py-4">@lang('app.tempSi')</th>
                            <th class="px-6 py-4">@lang('app.tempCo')</th>
                            <th class="px-6 py-4">@lang('app.zneCult')</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($datos as $cultivo)
                            <tr>
                                <td>{{ $cultivo->NomCult }}</td>
                                <td>{{ $cultivo->DesCult }}</td>
                                <td>{{ $cultivo->EstSiem }}</td>
                                <td>{{ $cultivo->EstCos }}</td>
                                <td>{{ $cultivo->ZonaCult }}</td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('cultivo.editar', $cultivo->idCult) }}">
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{ route('cultivo.borrar', $cultivo->idCult) }}" >
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
